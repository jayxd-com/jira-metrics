function fetchJiraData() {
  ...
  issues.forEach(function (issue) {
    ...
    var actualStoryPoints = issue.fields.parent
      ? issue.fields.parent.fields["customfield_10548"]
      : "";
    ...
    var sprintStartHuman = sprintStartDate
      ? moment(sprintStartDate).fromNow()
      : "";
    var sprintEndHuman = sprintEndDate ? moment(sprintEndDate).fromNow() : "";
    var sprintCompleteHuman = sprintCompleteDate
      ? moment(sprintCompleteDate).fromNow()
      : "";
...
function calculateVelocity(assigneeMap, projectSprintsMap) {
  var velocitySheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Velocity");
  if (!velocitySheet) {
    velocitySheet =
      SpreadsheetApp.getActiveSpreadsheet().insertSheet("Velocity");
  } else {
  ...
  for (var assignee in assigneePointsMap) {
    for (var sprint in assigneePointsMap[assignee]) {
      ...
      velocitySheet.appendRow([
        assignee,
        sprint,
        sprintData.initial,
        sprintData.completed,
        sprintData.rolledOver,
      ]);
    }
  }
}
